...
int bst_size(struct bst* bst) {
  ...
  int bst_node_size(struct bst_node* n){
    if(n==NULL){
      return 0;
    }
    else{
      return (bst_node_size(n->left) + 1 + bst_node_size(n->right));
    }
  }
...
int bst_node_height(struct bst_node* bst_node){
  if (!bst_node)
    return -1;  
  else{
    int lheight = bst_node_height(bst_node->left);
    int rheight = bst_node_height(bst_node->right);

    /* use the larger one */
    return max(lheight, rheight)+1;
  }

  //return 1 + max(bst_height(bst_node->left), bst_height(bst_node->right));
}
...
int bst_node_path_sum(struct bst_node* bst_node, int sum){
  if (bst_node ==NULL)
    return 0;
  if((bst_node->left==NULL) && (bst_node->right==NULL))
    return (bst_node->val==sum);

  return bst_node_path_sum(bst_node->left, sum - bst_node->val) || bst_node_path_sum(bst_node->right, sum - bst_node->val);
}
